# NOTES — g001-cgl-3d-b1p5-cm1p0-seed0001
...
```json
{
  ...
  "measured_by": {
    "variance_growth": 0.004285,
    "structure_factor_peak_k": 1.0,
    "structure_factor_prominence": 26.398877,
...
## EXPLORATION 所見
依頼2 [検証/探索]: (b,c)={'b': 1.5, 'c': -1.0}, t_final=100 (steps=5057, dt=0.0198), grid=(48, 48, 48)。3D の渦線再結合・結び目/絡み合いは common/diagnostics.winding_defect_count の貫通面カウント（線本数の下限プロキシ、docstring 記載）のみで評価しており、明示的な線追跡・結び目検出は未実装（未探索、依頼書の「足したら明記」に対応）。[探索] (b,c)=(1.5,-1.0), 1+bc=-0.50 (Benjamin-Feir 不安定域)。
